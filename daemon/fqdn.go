...
func identitiesForFQDNSelectorIPs(selectorsWithIPsToUpdate map[policyApi.FQDNSelector][]net.IP) (map[policyApi.FQDNSelector][]*identity.Identity, error) {
	...
	// Note that we do not need to update the prefixLengthCounter for
	// the Daemon because it will always contain /32 and /128 for IPv4
	// and IPv6. See daemon.go:createPrefixLengthCounter.
	ipNets := make([]*net.IPNet, 0)
	selectorIPNetMapping := make(map[policyApi.FQDNSelector][]*net.IPNet)
	for selector, netSlice := range selectorsWithIPsToUpdate {
		if prefixes := ip.GetCIDRPrefixesFromIPs(netSlice); prefixes != nil {
	...
	for selector, selectorIPNets := range selectorIPNetMapping {
		...
		if currentlyAllocatedIdentities, err = ipcache.AllocateCIDRsForIPs(selectorIPNets); err != nil {
