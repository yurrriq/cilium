...
func mergePortProto(ctx *SearchContext, existingFilter, filterToMerge *L4Filter, selectorCache *SelectorCache) error {
	...
	if existingFilter.AllowsAllAtL3() {
		...
		selectorCache.RemoveIdentitySelectors(filterToMerge, filterToMerge.CachedSelectors)
		...
		selectorCache.RemoveIdentitySelectors(existingFilter, existingFilter.CachedSelectors)
