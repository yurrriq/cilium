	...
	for _, rule := range selectingRules {
		for _, ingressRule := range rule.Ingress {
			if len(ingressRule.FromRequires) > 0 && len(ingressRule.FromEndpoints) == 0 {
				// Do not generate policy for rule that does not hat FromEndpoints coupled
				// with FromRequires.
				continue
			}
...
func computePortNetworkPolicy(portRules []api.PortRule, remotePolicies []uint64) []*cilium.PortNetworkPolicy {
	...
	if len(portRules) > 0 {
		...
		pnpTCP := cilium.PortNetworkPolicy{
		...
		portNetworkPolicies = append(portNetworkPolicies, &pnpTCP)
	}
...
func portRuleToPortNetworkPolicyRule(portRule api.PortRule, remotePolicies []uint64) []*cilium.PortNetworkPolicy {
	...
	for _, portProtocol := range portRule.Ports {
		...
		// There can be at most two protocols (TCP, UDP). Any protocol translates
		// to TCP and UDP.
