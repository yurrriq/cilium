...
func (e *EtcdLockedSuite) TestGetLocked(c *C) {
	randomPath := c.MkDir()
	type args struct {
		key  string
		lock KVLocker
	...
	for _, tt := range tests {
		...
		args := tt.setupArgs()
		want := tt.setupWanted()
		value, err := Client().GetLocked(args.key, args.lock)
...
func (e *EtcdLockedSuite) TestGetPrefixLocked(c *C) {
	randomPath := c.MkDir()
	type args struct {
		key  string
		lock KVLocker
	...
	for _, tt := range tests {
		...
		args := tt.setupArgs()
		want := tt.setupWanted()
		k, value, err := Client().GetPrefixLocked(context.Background(), args.key, args.lock)
		c.Assert(err, Equals, want.err)
...
func (e *EtcdLockedSuite) TestDeleteLocked(c *C) {
	randomPath := c.MkDir()
	type args struct {
		key  string
		lock KVLocker
	...
	for _, tt := range tests {
		...
		args := tt.setupArgs()
		want := tt.setupWanted()
		err := Client().DeleteLocked(args.key, args.lock)
		c.Assert(err, Equals, want.err)
		err = tt.cleanup(args)
		c.Assert(err, IsNil)
	}
...
func (e *EtcdLockedSuite) TestUpdateLocked(c *C) {
	randomPath := c.MkDir()
	type args struct {
		key      string
		lock     KVLocker
	...
	for _, tt := range tests {
		...
		args := tt.setupArgs()
		want := tt.setupWanted()
		err := Client().UpdateLocked(context.Background(), args.key, args.newValue, args.lease, args.lock)
		c.Assert(err, Equals, want.err)
		err = tt.cleanup(args)
		c.Assert(err, IsNil)
	}
...
func (e *EtcdLockedSuite) TestUpdateIfDifferentLocked(c *C) {
	randomPath := c.MkDir()
	type args struct {
		key      string
		lock     KVLocker
	...
	for _, tt := range tests {
		...
		args := tt.setupArgs()
		want := tt.setupWanted()
		updated, err := Client().UpdateIfDifferentLocked(context.Background(), args.key, args.newValue, args.lease, args.lock)
		c.Assert(err, Equals, want.err)
...
func (e *EtcdLockedSuite) TestCreateOnlyLocked(c *C) {
	randomPath := c.MkDir()
	type args struct {
		key      string
		lock     KVLocker
	...
	for _, tt := range tests {
		...
		args := tt.setupArgs()
		want := tt.setupWanted()
		created, err := Client().CreateOnlyLocked(context.Background(), args.key, args.newValue, args.lease, args.lock)
		c.Assert(err, Equals, want.err)
...
func (e *EtcdLockedSuite) TestListPrefixLocked(c *C) {
	randomPath := c.MkDir()
	type args struct {
		key  string
		lock KVLocker
	...
	for _, tt := range tests {
		...
		args := tt.setupArgs()
		want := tt.setupWanted()
		kvPairs, err := Client().ListPrefixLocked(args.key, args.lock)
		c.Assert(err, Equals, want.err)
