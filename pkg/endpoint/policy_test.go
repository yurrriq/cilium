...
type EndpointTestOwner struct {
	...
	OnQueueEndpointBuild      func(r *Request)
...
func (owner *EndpointTestOwner) QueueEndpointBuild(r *Request) {
	if owner.OnQueueEndpointBuild != nil {
		owner.OnQueueEndpointBuild(r)
		return
	}
...
func GenerateNumRules(numRules int) api.Rules {
	/*if numRules > 100000 {
		fmt.Printf("%d GREATER THAN 100,000\n", numRules)
		numRules = 300000
	}*/
	...
	fmt.Printf("done generating %d rules\n", numRules)
	return rules
}
