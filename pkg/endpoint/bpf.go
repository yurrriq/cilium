...
func hashEndpointHeaderfiles(prefix string) (string, error) {
	...
	epHeaderHashSum := hashWriter.Sum(nil)
	log.Errorf("epHeaderHashSum: %s", hex.EncodeToString(epHeaderHashSum[:]))
	...
	log.Errorf("combinedHeaderHashSum: %s", hex.EncodeToString(combinedHeaderHashSum[:]))
