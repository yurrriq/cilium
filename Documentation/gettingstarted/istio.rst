...
.. note::
   ...
   The hosts running Istio pods must have a bpf filesystem mounted
   (See :ref:`admin_mount_bpffs`).  If running on minikube, this can be
   done by issuing the following command *after* starting minikube, but
   *before* starting cilium:

::

    $ minikube ssh "sudo mount bpffs /sys/fs/bpf -t bpf"
