...
struct bpf_elf_map __section_maps cilium_cgroups_revnat4 = {
...
int sock4_xlate_snd(struct bpf_sock_addr *ctx)
{
	...
	if (svc) {
		...
		if (map_update_elem(&cilium_cgroups_revnat4, &rkey,
				    &rval, 0) < 0)
			return SENDMSG_PROCEED;
...
int sock4_xlate_rcv(struct bpf_sock_addr *ctx)
{
	...
	rval = map_lookup_elem(&cilium_cgroups_revnat4, &rkey);
	if (!rval) {
		...
		if (!svc || svc->rev_nat_index != rval->rev_nat_index) {
			map_delete_elem(&cilium_cgroups_revnat4, &rkey);
			return RECVMSG_PROCEED;
		}
...
struct bpf_elf_map __section_maps cilium_cgroups_revnat6 = {
	.type		= BPF_MAP_TYPE_LRU_HASH,
...
int sock6_xlate_snd(struct bpf_sock_addr *ctx)
{
	...
	if (svc) {
		...
		if (map_update_elem(&cilium_cgroups_revnat6, &rkey,
				    &rval, 0) < 0)
			return SENDMSG_PROCEED;
...
int sock6_xlate_rcv(struct bpf_sock_addr *ctx)
{
	...
	rval = map_lookup_elem(&cilium_cgroups_revnat6, &rkey);
	if (!rval) {
		return RECVMSG_PROCEED;
	} else {
		struct lb6_service_v2 *svc;
		...
		svc = __lb6_lookup_service_v2(&lkey);
		if (!svc || svc->rev_nat_index != rval->rev_nat_index) {
			map_delete_elem(&cilium_cgroups_revnat6, &rkey);
			return RECVMSG_PROCEED;
		}
