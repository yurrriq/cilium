...
func endpointsExists(endpoints format.Uint16Flags, existingEndpoints []*models.Endpoint) bool {
...
func validateEndpointsFilters() {
	...
	validFilter := false
	if len(printer.FromSource) > 0 {
		if endpointsExists(printer.FromSource, existingEndpoints) {
			validFilter = true
		}
	}
	if len(printer.ToDst) > 0 {
		if endpointsExists(printer.ToDst, existingEndpoints) {
			validFilter = true
		}
	}

	if len(printer.Related) > 0 {
		if endpointsExists(printer.Related, existingEndpoints) {
			validFilter = true
		}
	}
